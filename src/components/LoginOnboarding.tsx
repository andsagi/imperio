    ...
    <div id="login-screen-wrap" className="min-h-screen bg-[#121212] text-slate-100 flex flex-col items-center justify-center p-4 relative selection:bg-[#FF8C00] selection:text-black">
      ...
      >
        ...
        <div className="flex flex-col items-center text-center mb-6" id="login-brand-header">
          ...
          {/* Platform Differentials Section */}
          <div className="grid grid-cols-2 gap-2.5 mt-4 text-left w-full border-t border-neutral-850 pt-4" id="platform-differentials">
            <div className="flex items-start gap-1.5">
              <span className="text-[#FF8C00] font-bold text-xs mt-0.5">⚡</span>
              <div>
                <h4 className="text-[10px] font-black uppercase text-white tracking-wider">Atendimento Rápido</h4>
                <p className="text-[8.5px] text-slate-450 leading-tight">SOS de emergência acionado em minutos.</p>
              </div>
            </div>
            <div className="flex items-start gap-1.5">
              <span className="text-[#FF8C00] font-bold text-xs mt-0.5">🛡️</span>
              <div>
                <h4 className="text-[10px] font-black uppercase text-white tracking-wider">Rede Verificada</h4>
                <p className="text-[8.5px] text-slate-450 leading-tight">Apenas parceiros homologados.</p>
              </div>
            </div>
            <div className="flex items-start gap-1.5">
              <span className="text-[#FF8C00] font-bold text-xs mt-0.5">🤝</span>
              <div>
                <h4 className="text-[10px] font-black uppercase text-white tracking-wider">Negociação Direta</h4>
                <p className="text-[8.5px] text-slate-450 leading-tight">Fale com prestadores sem atravessador.</p>
              </div>
            </div>
            <div className="flex items-start gap-1.5">
              <span className="text-[#FF8C00] font-bold text-xs mt-0.5">💎</span>
              <div>
                <h4 className="text-[10px] font-black uppercase text-white tracking-wider">Sem Taxas</h4>
                <p className="text-[8.5px] text-slate-450 leading-tight">100% gratuito para motoristas.</p>
              </div>
            </div>
          </div>
        </div>
        ...
        {step === 1 ? (
          <div className="space-y-4">
            ...
            >
              <span>
                {role === 'seller' ? 'Avançar como Vendedor (Preencher Inscrição)' : role === 'supplier' ? 'Avançar como Proprietário (Preencher Inscrição)' : 'Avançar com Cadastro (Preencher Inscrição)'}
              </span>
              <ArrowRight className="w-4 h-4 stroke-[3.5]" />
            </button>
            <p className="text-[10px] text-slate-500 text-center leading-normal mt-1.5">
              Ao clicar no botão acima, você será direcionado ao formulário de inscrição para preenchimento dos dados cadastrais do perfil selecionado.
            </p>
          </div>
        ) : (
          <div className="space-y-4">
