...
export default function NicheSelector({ userName, onSelect, currentNiche }: NicheSelectorProps) {
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-8 md:py-12 flex flex-col items-center justify-center min-h-[85vh] animate-fadeIn" id="niche-selector-screen">
      ...
      <div className="mt-8 flex flex-col sm:flex-row items-center gap-4 text-[11px]">
        ...
        <button
          onClick={() => window.dispatchEvent(new CustomEvent('logout-trigger'))}
          className="px-4 py-1.5 bg-neutral-900 hover:bg-neutral-800 border border-neutral-800 text-slate-400 hover:text-white rounded-lg text-[10px] font-black uppercase tracking-wider transition-all cursor-pointer flex items-center gap-1"
        >
          <span>Voltar / Alterar Conta</span>
        </button>
      </div>
